...
In practice researchers are only ever dealing with estimates $\hat f_X$, $\hat f_Y$, $\hat f_X^{\text{MC}}$, and $\hat f_Y^{\text{MC}}$. The sets of data that contribute to these estimates are organized by bin into histograms that form an unnormalized granular approximation of the true distributions. Consider an example of the form described by Equation \eqref{eq:meas}, i.e. $Y_i=X_i+\varepsilon_i$. Let the error be i.i.d random variables generated by Gaussian noise, $\varepsilon_i\sim N(0,1)$, and let $X_i$ be a i.i.d. random variable from a composite distribution of the form $X_i=Z_iX_{1,i}+(1-Z_i)X_{2,i}$, where
\begin{align}
  X_{1,i}&\sim N(10,1),\nonumber\\
  X_{2,i}&\sim N(14,1),\nonumber\\
  \text{and }Z_i&\sim\text{Bernoulli}(1/3).\nonumber
\end{align}
Figure \ref{BasicExample} provides a visualization of the densities $f_X(x)$ and $f_Y(y)$, and their unnormalized approximations as histograms resulting from $10,000$ distributions.
\begin{figure}[ht]
    \centering
...
y <- y[y>0 & y<30]
y <- y[rbinom(length(y),1,sqrt(0.2*log(y+1)))==1]
xymin <- 0
xymax <- 30
xyaxisstep <- 3
...
XY <- seq(xymin,xymax,0.01)
nxy <- length(XY)

fig_data <- tibble(x = (1/3)*dcauchy(XY, location=loc1, scale=sc) + 
                     (2/3)*dcauchy(XY, location=loc2, scale=sc),
                   y = (1/3)*dcauchy(XY, location=loc1+ermu, scale=sqrt(sc+ersd)) + 
                     (2/3)*dcauchy(XY, location=loc2+ermu, scale=sqrt(sc+ersd)))

fig_data <- tibble(XY = rep(seq(xymin,xymax,0.01),2),
                           Density = c(fig_data$x,fig_data$y),
                           Treatment = rep(c("not folded","folded"), each = nxy))

fmax = max(fmax,ceiling(105*max(fig_data$Density))/100)

# Plot binned
p_step_notfolded <- ggplot(data = filter(step_hist,Treatment=="not folded")) + 
  theme_bw() +
  ...
  scale_y_continuous("Simulated Counts",
                     ...
                     breaks = seq(0,ceiling(fmax_count),floor(fmax_count/1000)*200)) +
...
p_step_folded <- ggplot(data = filter(step_hist,Treatment=="folded")) + 
  ...
  scale_y_continuous("Simulated Counts", 
                     limits = c(0,fmax_count), expand = c(0,0),
                     breaks = seq(0,ceiling(fmax_count),floor(fmax_count/1000)*200)) +
  labs(x="Y", y="", title=element_blank()) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.text.y = element_blank(), axis.ticks.y = element_blank(),
        axis.title.y = element_blank(), plot.margin = margin(0, 0.16, 0.12, 0.08, "cm"))

# Plot continuous
p_smooth_notfolded <- ggplot(filter(fig_data,Treatment=="not folded"), aes(XY, Density)) +
  geom_line(alpha=0.6, color="red") + theme_bw() + ggtitle("not folded") +
  scale_x_continuous(limits = c(xymin,xymax), expand = c(0, 0)) +
  scale_y_continuous("Density", 
                     seq(0,fmax,0.05), limits = c(0,fmax), expand = c(0, 0)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        axis.title.x = element_blank(), plot.margin = margin(0.08, 0.16, 0.04, 0.08, "cm"))

p_smooth_folded <- ggplot(filter(fig_data,Treatment=="folded"), aes(XY, Density)) +
...
grid.arrange(p_smooth_notfolded,p_smooth_folded,
             p_step_notfolded,p_step_folded,
             ncol=2, widths=c(12,10), heights=c(9,10))


```
\caption{\emph{A comparison of the true (not folded) distribution to the noisy (folded) distribution and the binned distributions of 10,000 corresponding simulated events.}}
\label{BasicExample}
\end{figure}

```{r, fig.height=2.4, fig.width=4, fig.align='center'}
ggplot(data = step_hist) + 
  theme_bw() +
  ...
  geom_segment(data = filter(step_hist, Treatment == "not folded"),
               mapping = aes(x = binLow,
                          ...
                          color = "Truth"), 
               alpha = 0.8) +
  geom_line(data = filter(fig_data, Treatment == "not folded"),
            mapping = aes(x = XY, 
                          y = nsim*Density,
  ...
  labs(x="X,Y", y="", title=element_blank()) +
...
For bin $i$ you would have
\begin{align}\nu_i = \sum_{j=1}^mR_{ij}\mu_j\label{eq:bini}\end{align}
\begin{align}R_{ij}&=P(\text{measured value in bin }i\vert\text{true value in bin }j)\nonumber\\&=P(\nu_i\vert\mu_j)\label{eq:Rij}\end{align}
\begin{align}
  \bm{R}=\begin{pmatrix}
    P(\nu_1\vert\mu_1)     & P(\nu_1\vert\mu_2)     & \dots  & P(\nu_1\vert\mu_{n})   \\
    P(\nu_2\vert\mu_1)     & P(\nu_2\vert\mu_2)     & \cdots & P(\nu_2\vert\mu_{n})   \\
    \vdots                 & \vdots                 & \ddots & \vdots                 \\
    P(\nu_{n}\vert\mu_1)   & P(\nu_{n}\vert\mu_2)   & \dots  & P(\nu_{n}\vert\mu_{n}) \\
    P(\nu_{n+1}\vert\mu_1) & P(\nu_{n+1}\vert\mu_2) & \dots  & P(\nu_{n+1}\vert\mu_{n})
  \end{pmatrix},\label{eq:Rmat}
\end{align}
such that
\begin{align}\sum_{i=1}^{n+1}P(\nu_i\vert\mu_j)=1\label{eq:norm}\end{align}
