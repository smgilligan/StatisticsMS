---
title: "Iterative Bayesian Approaches to Discrete Signal Deconvolution"
...
\begin{abstract}
This report discusses the use of a Bayesian approach for the unfolding (deconvolution) of experimental and observational data in order to form a better estimate of their true distribution. One approach is reasoned through and a poorly realized example is provided. Bayes Theorem is extensively implemented and sees heavy use of conjugate priors. Sampling from posteriors is used as an approach to explore both the development and propagation of uncertainty.
\end{abstract}

\section{Introduction}
A common problem faced in the quantitative sciences and their associated technologies is the introduction of errors during data collection processes. While the possible sources from which these errors might be derived are as varied as the possible events which the data might describe, statistical methods afford would-be analysts who wish to learn from their data the tools to recognize the sort of generalized structures that allow for the effective implementation of informative statistical models to this end.

One way to characterize a basic example with continuous variables would be the following. Assume that data collected regarding $n$ statistical events represent the measurement of $n$ independent and identically distributed (i.i.d.) random variables $\bm{X}=\{X_1,X_2,\dots,X_n\}$ from a distribution of possible values represented by the probability density function (PDF) $f(x)$, such that the probability of a random variable $X_i$ having a value between $x_a$ and $x_b$ is $$P(x_a<X_i<x_b)=\int_{x_a}^{x_b}f(x)dx$$ and $$\int_\mathcal{X}f(x)dx=1,$$ where $\mathcal{X}$ represents the domain of $x$. The error introduced during the measurement process is similarly represented by i.i.d. random variables $\bm{\varepsilon}=\{\varepsilon_1,\varepsilon_2,\dots,\varepsilon_n\}$ with a PDF $g(\varepsilon)$ and the typically assumed independence of the sets $\bm{\varepsilon}$ and $\bm{X}$. The set of measured values $\bm{Y}=\{Y_1,Y_2,\dots,Y_n\}$ then are also i.i.d. and can be defined in terms of the preceding sets of variables such that for event $i$, \begin{align}Y_i=X_i+\varepsilon_i.\label{eq:meas}\end{align} The corresponding PDF $h(y)$ can be found explicitly through an operation on $f(x)$ and $g(\varepsilon)$ using functional analysis. Stated in more general terms, the empirical density function $h$ is formed from the \textit{convolution} of the true density function $f$ and the error density function $g$, and is calculated by [@Meister2009] \begin{align}h&\equiv f*g\label{eq:conv1}\\h(y)&\equiv\int_\mathcal{X}f(x)g(\varepsilon)dx\nonumber\\&=\int_\mathcal{X}f(x)g(y-x)\left\vert\frac{\partial\varepsilon}{\partial x}\right\vert dx\nonumber\\&=\int_\mathcal{X}f(x)g(y-x)dx.\label{eq:conv2}\end{align}
For the purposes of integration $\varepsilon$ is treated as a function of $x$ for a given $y$ by the relation $\varepsilon=\varepsilon(x\vert y)=y-x$, as generated using the relation in Equation \eqref{eq:meas}. The term $\left\vert\frac{\partial\varepsilon}{\partial x}\right\vert$ then represents the Jacobian of the change of basis performed on $g$.

As an aside, it is worth noting that the integrand $f(x)g(y-x)$ in the final form of Equation \eqref{eq:conv2} can be collectively thought of as the joint probability distribution of $x$ and $y$, $P(x,y)$, with $g(y-x)$ serving as the conditional probability distribution of $y$ for a given $x$, $P(y\vert x)$. Here $P(\cdot)$ represents only probability density and acts as a mask to avoid referring to any particular distribution. Respectively relabeling $f(x)$ and $h(y)$ as the marginal probability distribution $P(x)$ and $P(y)$, the relationship between the marginal probability distribution and the joint probability distribution [@Gelman:2013bda] can be mapped to Equation \eqref{eq:conv2} as follows:\begin{align}P(y)&=\int_\mathcal{X}P(x,y)dx\nonumber\\&=\int_\mathcal{X}P(x)P(y\vert x)dx\nonumber\\h(y)&=\int_\mathcal{X}f(x)g(y-x)dx\nonumber\end{align}



While most literature on the subject does use the word \textit{convolution}, this operation is also referred to by the German word \textit{faltung} [@weisstein], and its English translation, \textit{folding}, is featured prominently in the particle physics community [@DAgostini:1994fjx][@DAgostini:2010hil][@Adye:2011gm][@Blobel:2013]. For the purposes of this paper instances of \textit{convolution} and \textit{folding} will refer to the same thing.



\begin{figure}[ht]
    \centering
```{r, fig.height=3.2, fig.width=4, fig.align='center', echo = F}
# Assume 10000 events
nsim <- 10000
...
step_hist <- 
  tibble(binLow = c(step_hist_folded$breaks[-length(step_hist_folded$breaks)],
         ...
         CountsL = c(c(0,step_hist_folded$counts[-length(step_hist_folded$counts)]),
                     c(0,step_hist_unfolded$counts[-length(step_hist_unfolded$counts)])),
         Counts = c(step_hist_folded$counts,step_hist_unfolded$counts),
...
```
\caption{\emph{The unfolded continuous distribution is of the form $X^{\text{unfolded}}_i= Z_i Y_{2,i}+(1-Z_i)Y_{1,i}$ where $Y_{1,i}\sim N(10,1)$, $Y_{2,i}\sim N(14,1)$, and $Z_i\sim\text{Bernoulli}(1/3)$. The folded continuous distribution is attained with the addition of simple Gaussian noise, such that $X^{\text{folded}}_i=X^{\text{unfolded}}_i+e_i$, where $e_i\sim N(0,1)$. The binned distributions consist of 10,000 simulated events from these distributions.}}
\label{BasicExample}
\end{figure}
...
The Bayesian methods described here were first proposed in the context of high energy physics in 1994 [@DAgostini:1994fjx] by Giulio D'Agostini, an Italian physicist and strong proponent of Bayesian reasoning in statistics. Later improvements to this method by him in 2010 [@DAgostini:2010hil] have lead to this iterative Bayesian approach becoming, along with methods based off Tikhonov regularization [@Hocker:1995kb][@Schmitt:2012kp], one of the major unfolding methods used in high energy physics today. To ease any potential reviewing of the reference material I will be adopting much of D'Agostini's notation and conventions, while also attempting to provide a more concise and clear description of his described methods.
