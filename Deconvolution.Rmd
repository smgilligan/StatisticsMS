---
title: "Iterative Bayesian Approaches to the Deconvolution of Discrete S in Experimental Particle Physics"
...
\begin{abstract}
This report discusses the use of iterative Bayesian approaches to
\end{abstract}
...
One way to characterize a basic example with continuous variables would be the following. Assume that data collected regarding $n$ statistical events represent the measurement of $n$ independent and identically distributed (i.i.d.) random variables $\bm{X}=\{X_1,X_2,\dots,X_n\}$ from a distribution of possible values represented by the probability density function (PDF) $f_X(x)$, such that the probability of a random variable $X_i$ having a value between $x_a$ and $x_b$ is $$P(x_a<X_i<x_b)=\int_{x_a}^{x_b}f(x)dx$$ and $$\int_\mathcal{X}f(x)dx=1,$$ where $\mathcal{X}$ represents the domain of $x$. The error introduced during the measurement process is similarly represented by a set of i.i.d. random variables $\bm{\varepsilon}=\{\varepsilon_1,\varepsilon_2,\dots,\varepsilon_n\}$ with a PDF $f_\varepsilon(\varepsilon)$, where the sets $\bm{\varepsilon}$ and $\bm{X}$ are typically assumed to be independent of each other. The set of measured values $\bm{Y}=\{Y_1,Y_2,\dots,Y_n\}$ then are also i.i.d. and can be defined in terms of the preceding sets of variables such that for event $i\in\{1,\dots,n\}$, \begin{align}Y_i=X_i+\varepsilon_i.\label{eq:meas}\end{align} In light of this relationship, the corresponding PDF $f_Y(y)$ can be found explicitly through an operation on $f_X(x)$ and $f_\varepsilon(\varepsilon)$ in the realm of functional analysis. Stated in more general terms, the empirical density function $f_Y$ is formed from the \textit{convolution} of the true density function $f_X$ and the error density function $f_\varepsilon$, and is defined by [@Meister2009] \begin{align}f_Y&\equiv f_X*f_\varepsilon\label{eq:conv1}\\f_Y(y)&\equiv\int_\mathcal{X}f_X(x)f_\varepsilon(\varepsilon)dx\nonumber\\&=\int_\mathcal{X}f_X(x)f_\varepsilon(y-x)\left\vert\frac{\partial\varepsilon}{\partial x}\right\vert dx\nonumber\\&=\int_\mathcal{X}f_X(x)f_\varepsilon(y-x)dx.\label{eq:conv2}\end{align}
For the purposes of integration $\varepsilon$ is treated as a function of $x$ and $y$ by the relation $\varepsilon=\varepsilon(x,y)=y-x$, as generated using the relation in Equation \eqref{eq:meas}. For a constant $y$, the term $\left\vert\frac{\partial\varepsilon}{\partial x}\right\vert$ then represents the Jacobian of the change of basis performed on $f_\varepsilon$, which happens to be $1$ here.

\comment{
As an aside, it is worth noting that the integrand $f_X(x)g_\varepsilon(y-x)$ in the final form of Equation \eqref{eq:conv2} can be collectively thought of as the joint probability distribution of $x$ and $y$, $f_{X,Y}(x,y)$, with $f_\varepsilon(y-x)$ serving as the conditional probability distribution of $y$ for a given $x$, $f_{\varepsilon}(y\vert x)$. The relationship between the marginal probability distribution and the joint probability distribution [@Gelman:2013bda] can be mapped to Equation \eqref{eq:conv2} as follows:\begin{align}P(y)&=\int_\mathcal{X}P(x,y)dx\nonumber\\&=\int_\mathcal{X}P(x)P(y\vert x)dx\nonumber\\h(y)&=\int_\mathcal{X}f(x)g(y-x)dx\nonumber\end{align}
}

As the collection of measured values $\bm{Y}$ accumulates an estimate of empirical density $\hat{f}_Y$ can readily be formed. However, a major goal in an analysis of data like this is typically to develop an accurate estimate of the true density $\hat{f}_X$. Using the information contained in $\hat{f}_Y$ to accomplish this necessarily requires some attempt at finding an inverse process to the \textit{convolution}, i.e. the \textit{deconvolution}. What this might look like depends heavily on the relevant assumptions and knowledge held by the analysts, such as information about the processes that introduce error.

The remainder of this paper is dedicated to characterizing the major methods adopted by the High Energy Physics (HEP) community in this endeavor. While most literature on the subject does use the word \textit{convolution}, this operation is also referred to by the German word \textit{faltung} [@weisstein], and its English translation, \textit{folding}, is featured prominently in the particle physics community [@DAgostini:1994fjx][@DAgostini:2010hil][@Adye:2011gm][@Blobel:2013]. For the purposes of this paper instances of \textit{folding} and \textit{unfolding} will refer to \textit{convolution} and \textit{deconvolution}, and vice-versa.
