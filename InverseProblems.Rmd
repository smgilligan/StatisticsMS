...
It will be convenient, and necessary for methods based on least squares, to estimate the
...
\begin{figure}[!ht]
    \centering
```{r, fig.height=2.5, fig.width=7.5, fig.align='center', echo = F}
...
(or pseudo-inverse) \cite{Blobel2013} of $\bm{R}$. If $\bm{R}^T\bm{\Sigma}_{\nu}^{-1}\bm{R}\bm{\mu}$ is singular this approach will not work, as its inverse corresponds to the estimated true counts' covariance matrix, as shown below.
\begin{align}
  \bm{\Sigma}_\mu&=\text{Cov}[\bm{\hat\mu}]=\text{Cov}[\bm{R}^{+}\bm{n}]=\bm{R}^{+}\text{Cov}[\bm{n}]\bm{R}^{+^T}=\bm{R}^{+}\bm{\Sigma}_\nu\bm{R}^{+^T}\nonumber.
\end{align} 
This is the same covariance matrix shown in Figure \eqref{CRlb}, which indicates that $\bm{R}^{+}$ is an unbiased estimator that should have lowest possible variances among unbiased estimators. For the simulations one such inverse matrix was calculated from the response matrix of each MC Model simulation, and was used to form an estimate of $\bm{\Sigma}_\mu$ for each MC-Data combination using each Data model's estimated covariance matrix. 
...
The massive, rapid oscillations and large uncertainties indicate that these are not very good results, regardless of whether or not the Data and the MC came from the same Model. The unfolding process used here cannot account for even small random variation in Reconstructed space because it has been over-fitted by the MC results. In search of an unbiased estimator we have unwittingly created a minimum variance that is simply too large be in any way useful. Some degree of bias must be allowed to balance it. In order to accomplish this an estimated $\bm{\hat\mu}$ must be chosen to probe the region around the minimum least-squares of Equation \eqref{eq:leastsq} (or equivalent maximum log-likelihood) to find an alternative solution that can actually be used on other data. This is done by including an extra term in these equations through a process called \emph{Regularization}.

\subsection{Regularization}

This section reviews a method of regularized unfolding that is unfortunately only really referred to in particle physics as just \emph{Singular Value Decomposition} \cite{Adye2011}\cite{Blobel2013}\ref{Cowan1998}, which is also a matrix factorization method in linear algebra. To be fair, this unfolding method does prominently use it. As such, singular value decomposition (SVD) involves the factorization of an $N\times M$ matrix $\bm{A}$ into
\begin{align}
  \bm{A}=\bm{USV}^T\label{SVD}
\end{align}
...
#B1 <- Vnu1
#B2 <- Vnu2
B1 <- diag(dataReco1$Counts)
B2 <- diag(dataReco2$Counts)
...
As1 <- t(t(Q1 %*% A1) %*% solve(R1))
As2 <- t(t(Q2 %*% A2) %*% solve(R2))

bs11 <- t(t(Q1 %*% b1) %*% solve(R1))
bs12 <- t(t(Q1 %*% b2) %*% solve(R1))
bs21 <- t(t(Q2 %*% b1) %*% solve(R2))
bs22 <- t(t(Q2 %*% b2) %*% solve(R2))

Us1 <- svd(As1 %*% solve(C))$u
Us2 <- svd(As2 %*% solve(C))$u
Vs1 <- svd(As1 %*% solve(C))$v
Vs2 <- svd(As2 %*% solve(C))$v
Ss1 <- diag(svd(As1 %*% solve(C))$d)
Ss2 <- diag(svd(As2 %*% solve(C))$d)
s1 <- diag(Ss1)
s2 <- diag(Ss2)
...
mu11hats <- map(taus,~(solve(C)%*%Vs1%*%(d11*(s1/(s1^2+.))))*mcTruth1$Counts)
mu12hats <- map(taus,~(solve(C)%*%Vs1%*%(d12*(s1/(s1^2+.))))*mcTruth1$Counts)
mu21hats <- map(taus,~(solve(C)%*%Vs2%*%(d21*(s2/(s2^2+.))))*mcTruth2$Counts)
mu22hats <- map(taus,~(solve(C)%*%Vs2%*%(d22*(s2/(s2^2+.))))*mcTruth2$Counts)

W1s <- map(taus, ~solve(C)%*%Vs1%*%diag(s1^2/(s1^2+.))
           %*%t(Vs1)%*%solve(t(C)))
W2s <- map(taus, ~solve(C)%*%Vs2%*%diag(s2^2/(s2^2+.))
           %*%t(Vs2)%*%solve(t(C)))

mu1 <- mcTruth1$Counts
mu2 <- mcTruth2$Counts

X1s <- matrix(map2_dbl(rep(1:30,each=30),rep(1:30,30),
                       ~(As1[,.x]%*%As1[,.y])/(mu1[.x]*mu1[.y])),
              nrow = 30,byrow = T)
X2s <- matrix(map2_dbl(rep(1:30,each=30),rep(1:30,30),
                       ~(As2[,.x]%*%As2[,.y])/(mu2[.x]*mu2[.y])),
              nrow = 30,byrow = T)

chi11s <-map_dbl(1:length(taus), ~ t(mu11hats[[.]] - mu1/dmrat) 
                 %*% X1s %*%( mu11hats[[.]] - mu1/dmrat))
chi12s <-map_dbl(1:length(taus), ~ t(mu12hats[[.]] - mu1/dmrat) 
                 %*% X1s %*%( mu12hats[[.]] - mu1/dmrat))
chi21s <-map_dbl(1:length(taus), ~ t(mu21hats[[.]] - mu2/dmrat) 
                 %*% X1s %*%( mu21hats[[.]] - mu2/dmrat))
chi22s <-map_dbl(1:length(taus), ~ t(mu22hats[[.]] - mu2/dmrat) 
                 %*% X1s %*%( mu22hats[[.]] - mu2/dmrat))

compareChis <- 
  tibble(tau = rep(taus,4), 
         Chi2 = c(chi11s,chi12s,chi21s,chi22s),
         col=rep(c("MC1 Data1","MC1 Data2",
                   "MC2 Data1","MC2 Data2"),each=length(taus)))

ggplot(compareChis, aes(x=tau, y=Chi2, color=col)) +
