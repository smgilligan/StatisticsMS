...
comp1 <- ggplot(data = step_hist) + 
  ...
  scale_color_manual(name = NA,
                     breaks = c("E[Binned Truth]",
                                ...
                                "E[Binned Measured]",
                                "PDF Measured"),
                     values = c("E[Binned Truth]" = "#4C8C2B",
                                "PDF Truth" = "#78BE20",
                                "E[Binned Measured]" = "#004C97",
                                "PDF Measured" = "#00B5E2")) +
...
migration1 <- ggplot(filter(sims, Truth <= 30 & Truth >= 0 &
...
migration2 <- ggplot(filter(sims, Truth <= 30 & Truth >= 0 &
                              Measured <= 30 & Measured >= 0 &
                              Detected == FALSE)) +
  scale_x_continuous(breaks = seq(xymin,xymax,xyaxisstep), 
                     limits = c(xymin,xymax), 
  ...
  stat_bin2d(mapping = aes(x = Truth, 
             ...
             breaks = list(x = seq(0,30,1),
                           y = seq(0,30,1))) +
  labs(x = "X (Truth)", y = "Y (Measured)",
       title = "Events Not Detected")

grid.arrange(comp1, migration1, comp2, migration2, ncol = 2, widths = c(6,5))
```
