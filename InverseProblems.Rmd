...
\begin{align}
  \mu(X_i=x)&=-2\big(\log(\vert x\vert+1)\big)^{1/3}\;\;\text{and}\nonumber\\
  \sigma(X_i=x)&=2\exp\big(-\vert x\vert/30\big).\nonumber
\end{align}
...
\begin{align}
  p(X_i=x)=\left(1-e^{-\vert x\vert/80}\right)^{1/4}.\nonumber
\end{align}
\footnote{Here is some food for thought. In particle physics the two Cauchy processes contributing to $X_i$ would likely represent two very different physics processes with different numbers and types of information carriers. For some physics variable of interest the detection and measurement process to reconstruct a possible measured value of $Y_i$ from an event with a true value of $X_i$ would vary accordingly, suggesting the replacement of $\varepsilon_i$ and $\epsilon_i$ with pairs $\{\varepsilon_{1,i},\varepsilon_{2,i}\}$ and $\{\epsilon_{1,i},\epsilon_{2,i}\}$, and altering the form of the example to
\begin{align}Y_i&=Z_iY_{1,i}+(1-Z_i)Y_{2,i}\nonumber\\&=Z_i\epsilon_{1,i}\left(X_{1,i}+\varepsilon_{1,i}\right)+(1-Z_i)\epsilon_{2,i}\left(X_{2,i}+\varepsilon_{2,i}\right).\nonumber\end{align}}
...
x <- z*x1 + (1-z)*x2
# Efficiency
xdetected <- rbernoulli(length(x),(1-exp(-abs(x)/80))^(1/4)) == 1
...
comp1 <- ggplot(data = step_hist) + 
  ...
  scale_x_continuous(breaks = seq(xymin-dxy,xymax+dxy,xyaxisstep), 
...
migration <- sims %>%
  ...
  ggplot() +
    scale_x_continuous(breaks = seq(xymin-dxy,xymax+dxy,xyaxisstep),
                       limits = c(xymin-dxy,xymax+dxy), 
                       expand = c(0,0)) +
    scale_y_continuous(breaks = seq(xymin-dxy,xymax+dxy,xyaxisstep), 
                       limits = c(xymin-dxy,xymax+dxy), 
                       expand = c(0,0)) +
    ...
    geom_rect(data=tibble(x1=xymin,x2=xymax,y1=xymin,y2=xymax),
              mapping=aes(xmin=x1,xmax=x2,ymin=y1,ymax=y2),
              alpha=0,col="red",size=0.3) +
    geom_rect(data=tibble(x1=xymin-dxy,x2=xymin,y1=xymin-dxy,y2=xymin),
              mapping=aes(xmin=x1,xmax=x2,ymin=y1,ymax=y2),
              alpha=0.5,col="red",size=0.3) +
    geom_rect(data=tibble(x1=xymax,x2=xymax+dxy,y1=xymax,y2=xymax+dxy),
              mapping=aes(xmin=x1,xmax=x2,ymin=y1,ymax=y2),
              alpha=0.5,col="red",size=0.3) +
    geom_rect(data=tibble(x1=xymax,x2=xymax+dxy,y1=xymin-dxy,y2=xymin),
              mapping=aes(xmin=x1,xmax=x2,ymin=y1,ymax=y2),
              alpha=0.5,col="red",size=0.3) +
    geom_rect(data=tibble(x1=xymin-dxy,x2=xymin,y1=xymax,y2=xymax+dxy),
              mapping=aes(xmin=x1,xmax=x2,ymin=y1,ymax=y2),
              alpha=0.5,col="red",size=0.3) +
