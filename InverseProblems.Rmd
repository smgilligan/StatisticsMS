...
migration <- sims_mc %>%
  ...
  ggplot() +
    scale_y_continuous(breaks = seq(0,30,by=3),
                       limits = c(0,30), 
                       expand = c(0,0)) +
    scale_x_continuous(breaks = seq(-3,33,by=3), 
                       limits = c(-4,35), 
                       expand = c(0,0)) +
    theme_bw() +
    stat_bin2d(mapping = aes(x = Reconstructed, 
                             y = Truth), 
               breaks = list(x = seq(-4,35,1),
                             y = seq(0,30,1))) +
    labs(x = "Y (Reconstructed)", y = "X (Truth)",
         title = "MC Migration") +
    geom_abline(intercept=0,slope=1,lty="dashed",col="red") +
    facet_wrap(Theory~., ncol = 2)
...
biases <- c((mcTruth1$Counts/mcReco1$Counts[1:30] - 
  ...
  (bins_expected2 %>% filter(Treatment == "Measured") %>% pull(Counts)),
  (mcTruth2$Counts/mcReco1$Counts[1:30] - 
  (bins_expected1 %>% filter(Treatment == "Truth") %>% pull(Counts))/
  (bins_expected1 %>% filter(Treatment == "Measured") %>% pull(Counts))) *
  (bins_expected1 %>% filter(Treatment == "Measured") %>% pull(Counts)),
  (mcTruth2$Counts/mcReco1$Counts[1:30] - 
...
plot(c(-1,31), c(0,0), lty=2, ylim=c(min(biases),max(biases)), type="l")
lines(1:30, biases[1:30], col=2)
lines(1:30, biases[31:60], col=3)
...
bins_hat <- tibble(Theory = c(rep(c(rep(c("Theory 1","Theory 2","Theory 3"),each=30),
                   ...
                   Name = c(rep("Theory 1 Reconstucted Data",30),
                            rep("Theory 2 Reconstucted Data",30),
                            rep("Theory 3 Reconstucted Data",30),
                            rep("Theory 1 Reconstucted MC",30),
                            rep("Theory 2 Reconstucted MC",30),
                            rep("Theory 1 Truth Data",30),
                            rep("Theory 2 Truth Data",30),
                            rep("Theory 3 Truth Data",30),
                            rep("Theory 1 Truth MC",30),
                            rep("Theory 2 Truth MC",30),
                            rep("Theory 1 Unfolded Data",60),
                            rep("Theory 2 Unfolded Data",60),
                            rep("Theory 3 Unfolded Data",60),
                            rep("Theory 1 Unfolded MC",60),
                            rep("Theory 2 Unfolded MC",60)),
