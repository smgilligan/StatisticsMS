...
continuous2 <- ggplot() + 
  ...
  geom_line(data = rbind(fx_truth1,fx_truth2,
            ...
            alpha = 0.7) +
  scale_color_manual(name = "Theory 2 MC",
                     labels = c("Truth",
                                "Reconstructed"),
                     breaks = c("Truth",
                                "Reconstructed"),
  ...
  scale_linetype_manual(name = NA,
                        ...
                        values = c("Truth" = "solid",
                                   "Reconstructed" = "solid")) +
  guides(color = guide_legend(title = "Theory 1 MC", order = 1, 
...
discrete_DataMC <- ggplot() + 
  ...
  guides(color = guide_legend(title = "Theory 1 MC", order = 1, 
         ...
         linetype = guide_legend(title = "Theory 2 MC", order = 2, 
                                 override.aes = 
...
```
  \caption{\emph{\small The distinguishing characteristics between Theory 1 and Theory 2 become greatly diminished as their respective distributions are smeared and diffused by shared phenomenon. Ignoring the knowledge granted in this figure's legend, one would be hardpressed to predict which Truth distribution a particular Reconstructed distribution came from.}}
  \label{plots1}
\end{figure}

Please see Appendix \ref{simuApp} for more specific information about the performed simulations. Referring to Figure \ref{plots1}, the blue and red colored plots correspond to the true distributions of Theory 1 and Theory 2 respectively. The colored lines represent the MC simulations. They have been rescaled to represent the same number of possible events as the experimental data. The impact of the third contributing Cauchy process in Theory 2 can be seen by way of the notably diminished peak and the partial filling of the dividing indent between the two processes of Theory 1. While it is clear for both the continuous and discrete representations that the two larger distributions are distinct, less can be said about them once imperfect detector efficiency and resolution have had their effects.

\begin{figure}[!ht]
    \centering
```{r, fig.align='center', echo = F, fig.height=3.5, fig.align='center'}
########## MIGRATION ###########################################################
...
Figure \ref{plots2} provides an alternative perspective to Figure \ref{plots1} by providing a visual representation of the two theories' migration matrices. The red dashed lines indicate where events would live if there were no skewing or smearing processes. Note that the Reconstructed axis extends past points covered by the Truth axis, providing insight into the behavior with which events produced in the primary region of interest ends up measured or reconstructed outside this so-called \textbf{fiducial volume}.
...
The international particle physics community is inherently collaborative
...
(or pseudo-inverse) \cite{Blobel2013} of $\bm{R}$. It is assumed that
