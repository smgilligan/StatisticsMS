...
header-includes:
  ...
  - \newcommand{\comment}[1]{}
abstract: \singlespacing This report provides an a brief look at ill-posed inverse problems in experimental particle physics as they pertain to signal distortions that result from imperfect measuring devices and processes. The methods they use in solving them are in general collectively referred to as unfolding. The specifics of data and data collection methods are generalized. Common features are discussed insofar as they contribute to the necessary understanding of the data and implementation of any covered methods. In order to construct a slightly more wholistic picture some additional topics are briefly touched upon if they relate to other common aspects of data analysis in particle physics, but only during parts of relevant discussions where they would otherwise normally appear. One non-iterative unfolding procedure is discussed and applied to an example.
fontsize: 12pt
editor_options: 
  markdown: 
    wrap: 72
...
chis11d <- chis12d <- chis21d <- chis22d <- 
  chis11m <- chis12m <- chis21m <- chis22m <- rep(0,nt1+nt2+ntS)
...
\begin{tabular}{| c | c || c | c || c | c |} 
 \hline
 MC & Data & $\tau=s_5^2$ & $\chi^2$ & $\min\limits_{\tau}\sum\chi^2$ & $\chi^2$ \\
 \hline
 1 & 1 & $5029.04$ &  $3.61$ & $4386.22$ &  $3.63$ \\ 
 1 & 2 & $4942.46$ & $11.60$ & $4386.22$ & $11.43$ \\
 2 & 1 & $4972.61$ &  $2.14$ & $4386.22$ &  $1.93$ \\
 2 & 2 & $4923.57$ &  $5.98$ & $4386.22$ &  $6.27$ \\
 \hline
 \multicolumn{2}{|c||}{All} & \multicolumn{2}{|r||}{$\sum\chi^2=23.33$} & \multicolumn{2}{|r|}{$\sum\chi^2=23.26$}\\
 \hline
\end{tabular}
\caption{The $\chi^2$s and their summations $\sum\chi^2$ for the listed values of $\tau$ as chosen by their respective selection methods.}
\label{tab:chi2}
\end{table}
...
k <- 5
tau <- taus[1,1,(nt1+1):(nt1+nt2)][which.min((chis11d+chis12d+
                                                chis21d+chis22d)[(nt1+1):(nt1+nt2)])]
...
unfplot_t <- ggplot() + theme_bw() + geom_hline(yintercept = 0, lty = 2) +
  ...
  labs(title = expression(paste("SVD Unfolded (",tau==4386.22,")"))) +
  facet_wrap(~label)
...
unfplot_t
```
\caption{\emph{\small Unfolded Data superimposed on both MC Truth histograms. Data from Model 2 had the more difficult time unfolding using the opposite Model's MC response matrix during unfolding.}}
  \label{unfolded}
\end{figure}

In particular, when both sets of Data are paired with the wrong MC model during unfolding, they both consistently behave similarly across the observed regions of $\tau$ values, and reach a minimum at a value of $\tau$ that aligns well with confirmatory $\chi^2$ values for the same Data when paired with the correct MC models. As a contrast to finding $k$ as depicted in Figure \ref{TauSelect}, these observations inform a method for choosing the value of $\tau$ which minimizes the sum of squares of differences between the unfolded counts of test Data and their true counts. A side-by-side numerical comparison of both methods' sums of squares is given in Table \ref{tab:chi2}.

In Figure \ref{unfolded} each of the unfolded results are plotted against both MC simulations Truth counts. In all cases it is apparent that the Data sets unfolded more into agreement with the MC histograms that were simulated with the same model, regardless of which MC set was used in the unfolding process.

\section{Conclusion}

The SVD-based unfolding method described here provides a robust approach to solving ill-posed inverse problems.
...
diffdat1 <- hats %>% filter((MC == "MC 1" & Data == "Data 1" & Taus == tau) |
                  ...
                  (MC == "MC 2" & Data == "Data 1" & Taus == tau) |
                  (MC == "MC 2" & Data == "Data 2" & Taus == tau)) %>%
  mutate(DataTruth = rep(c(dataTruth1$Counts,dataTruth2$Counts),2))

areadat1 <- tibble(X = rep(c(0,rep(1:30,each=2),rep(29:1,each=2),0,0),4),
                  Y = rep(c(rep(dataTruth1$Counts,each=2),-rev(rep(dataTruth1$Counts,each=2)),dataTruth1$Counts[1],
                            rep(dataTruth2$Counts,each=2),-rev(rep(dataTruth2$Counts,each=2)),dataTruth2$Counts[1]),2),
                  label = rep(c("MC 1 / 1 Data - 1 Data","MC 1 / 2 Data - 2 Data",
                                "MC 2 / 1 Data - 1 Data","MC 2 / 2 Data - 2 Data"),each=121),
                  Data = rep(c(rep("Data 1",121),rep("Data 2",121)),2))

g1 <- ggplot() + theme_bw() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_polygon(data = areadat1,
            mapping = aes(x=X,y=sign(Y)*sqrt(abs(Y)),fill=Data,color=Data),
            linetype = "dotted") +
  geom_polygon(data = areadat1,
            mapping = aes(x=X,y=2*sign(Y)*sqrt(abs(Y)),fill=Data,color=Data),
            linetype = "dotted") +
  geom_errorbar(data = diffdat1,
                mapping = aes(x=X,ymin=DataTruth-CVCounts-SD,
                              ymax=DataTruth-CVCounts+SD),width=0.2) +
  geom_point(data = rbind(diffdat1,diffdat1,diffdat1),
             mapping = aes(x=X,y=DataTruth-CVCounts,fill=Data), shape = 21,size=1.5) +
  ...
  scale_x_continuous(expand = c(0,0), breaks = seq(0,30,3),
                     name = "X (Truth)") +
  scale_y_continuous(name = TeX("$\\mu-\\hat{\\mu}$    Count")) +
...
diffdat2 <- hats %>% filter((MC == "MC 1" & Data == "Data 1" & Taus == tau) |
                  ...
                  (MC == "MC 2" & Data == "Data 1" & Taus == tau) |
                  (MC == "MC 2" & Data == "Data 2" & Taus == tau)) %>%
  mutate(DataTruth = rep(c(dataTruth2$Counts,dataTruth1$Counts),2))

areadat2 <- tibble(X = rep(c(0,rep(1:30,each=2),rep(29:1,each=2),0,0),4),
                  Y = rep(c(rep(dataTruth2$Counts,each=2),-rev(rep(dataTruth2$Counts,each=2)),dataTruth2$Counts[1],
                            rep(dataTruth1$Counts,each=2),-rev(rep(dataTruth1$Counts,each=2)),dataTruth1$Counts[1]),2),
                  label = rep(c("MC 1 / 1 Data - 2 Data","MC 1 / 2 Data - 1 Data",
                                "MC 2 / 1 Data - 2 Data","MC 2 / 2 Data - 1 Data"),each=121),
                  Data = rep(c(rep("Data 2",121),rep("Data 1",121)),2))

diffdat1$label[which(diffdat1$label=="MC 1 / 1 Data")] <-
  paste0(diffdat1$label[which(diffdat1$label=="MC 1 / 1 Data")]," - 1 Data")
diffdat1$label[which(diffdat1$label=="MC 1 / 2 Data")] <-
  paste0(diffdat1$label[which(diffdat1$label=="MC 1 / 2 Data")]," - 2 Data")
diffdat1$label[which(diffdat1$label=="MC 2 / 1 Data")] <-
  paste0(diffdat1$label[which(diffdat1$label=="MC 2 / 1 Data")]," - 1 Data")
diffdat1$label[which(diffdat1$label=="MC 2 / 2 Data")] <-
  paste0(diffdat1$label[which(diffdat1$label=="MC 2 / 2 Data")]," - 2 Data")
diffdat2$label[which(diffdat2$label=="MC 1 / 1 Data")] <-
  paste0(diffdat2$label[which(diffdat2$label=="MC 1 / 1 Data")]," - 2 Data")
diffdat2$label[which(diffdat2$label=="MC 1 / 2 Data")] <-
  paste0(diffdat2$label[which(diffdat2$label=="MC 1 / 2 Data")]," - 1 Data")
diffdat2$label[which(diffdat2$label=="MC 2 / 1 Data")] <-
  paste0(diffdat2$label[which(diffdat2$label=="MC 2 / 1 Data")]," - 2 Data")
diffdat2$label[which(diffdat2$label=="MC 2 / 2 Data")] <-
  paste0(diffdat2$label[which(diffdat2$label=="MC 2 / 2 Data")]," - 1 Data")
diffdat <- rbind(diffdat1,diffdat2)

ggplot() + theme_bw() +
  geom_hline(yintercept = 0, linetype = "dashed") + 
  geom_polygon(data = rbind(areadat1,areadat2),
            mapping = aes(x=X,y=sign(Y)*sqrt(abs(Y)),fill=Data,color=Data),
            linetype = "dotted") +
  geom_polygon(data = rbind(areadat1,areadat2),
            mapping = aes(x=X,y=2*sign(Y)*sqrt(abs(Y)),fill=Data,color=Data),
            linetype = "dotted") +
  geom_errorbar(data = rbind(diffdat1,diffdat2),
                mapping = aes(x=X,ymin=DataTruth-CVCounts-SD,
                              ymax=DataTruth-CVCounts+SD),width=0.2) +
  geom_point(data = rbind(diffdat1,diffdat1,diffdat1,diffdat2,diffdat2,diffdat2),
             mapping = aes(x=X,y=DataTruth-CVCounts,fill=Data), shape = 21,size=1.5) +
  scale_color_manual(values = c("#5e81b5","#e19c24")) +
  scale_fill_manual(values = c(alpha("#5e81b5",0.3),alpha("#e19c24",0.3))) +
  scale_x_continuous(expand = c(0,0), breaks = seq(0,30,3),
