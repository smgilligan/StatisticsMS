...
For the sake of simplicity, the manner of inverse problems addressed here will only involve linear operations between two Hilbert spaces\footnote{The definition of a Hilbert space is provided in Appendix \ref{appHilbert} for convenience}. Symbolically this can be expressed by the equation
\begin{align}
Az=u,\nonumber
\end{align}
where $A$ is a linear operator mapping the operand $z$, the sought "solution", in some Hilbert space $Z$ to $u$, the "initial data", in some Hilbert space $U$.
...
The following definitions come from Rudin in \cite{Rudin1991}. A couple additional definitions are in order, as Hilbert spaces exist at the intersection of several other important vector spaces. To start, a \textbf{vector space}, as defined here, consists of a set $X$ of vectors for which addition and scalar multiplication are defined such that for all $x,y,z\in X$ and any complex number $\alpha\in\mathbb{C}$
\begin{enumerate}
  \item There exists a vector in $X$ such that
    ...
    \end{enumerate}
  \item $\alpha x$ exists in $X$ such that $1x=x$, $0x=0$ (the $0$ vector), and multiplication is distributive:
    \begin{enumerate}
...
An important set of spaces that intersects with the set of all vector spaces are the metric spaces. A space $X$ is a \textbf{metric space} if for all $x,y\in X$ there exists an operator $d(x,y)$ that maps them to a nonnegative real number that defines their distance from each other within $X$. The properties of this operator are
\begin{enumerate}
...
\end{enumerate}
The distance operator here $d$ is referred to as the metric on $X$. The intersection of vector and metric spaces form the set of normed spaces. As extension of the conditions thus far, a space $X$ is a \textbf{normed space} if $\forall x\in X$ there exists a nonnegative real number $\vert\vert x\vert\vert$, called the \textbf{norm} of $x$ such that
\begin{enumerate}
...
d(x_m,x_n)
  ...
  &<\frac{1}{\sqrt{n}}\left(\frac{\delta}{2}\right),\nonumber
\end{align}
for which the limit as $n\longrightarrow\infty$ is the $0$ vector.
...
venndiagram <- tibble(X0 = c(0,0,-1.2,0.9),
                      Y0 = c(-0.75,0.75,0,-1.05),
                      R = c(4.5,4.5,2.75,2.3),
                      space = c("Vector Space", "Metric Space", "Inner Product Space",
                                "Banach Space")) %>%
  mutate(across(space, factor, 
                levels = c("Vector Space","Metric Space","Normed Space",
                           "Inner Product Space","Banach Space", "Hilbert Space")))
venntext <- tibble(X = c(0,2,0,-2.2,1.8,0),
                   Y = c(4.5,2,-4.5,0.75,-1.8,-0.6),
                   theta = c(0,-45,0,50,55,0),
                   text = c("Vector Space\n(vectors)","Normed Space\n(length)",
...
ggplot() +
  ...
  scale_fill_manual(values = c("#a01000", "#409500", "#e55500", "#000090", "#ffffff"))
